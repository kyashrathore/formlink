...
async function runTail(sandbox, filesPattern) {
  ...
  sandbox
    .runCommand({ cmd: cmd[0], args: cmd.slice(1), stdout: process.stdout, stderr: process.stderr })
    .catch(() => {})
}
...
async function main() {
  ...
  if (!args.id) {
    console.error('Usage: sandbox-terminal --id <sandboxId> [--team <teamId>] [--project <projectId>] [--token <token>] [--no-tail]')
    process.exit(1)
  }
  ...
  if (!token || !teamId || !projectId) {
    console.error('Missing sandbox credentials. Provide --token/--team/--project or set SANDBOX_VERCEL_TOKEN (or VERCEL_OIDC_TOKEN), SANDBOX_VERCEL_TEAM_ID, SANDBOX_VERCEL_PROJECT_ID')
    process.exit(1)
  }

  console.log(`[sbx] connecting to sandbox ${args.id} (team=${teamId}, project=${projectId})`)
  let sandbox
  try {
    sandbox = await Sandbox.get({ sandboxId: args.id, teamId, projectId, token })
  } catch (err) {
  ...
  if (!args.noTail) {
    console.log('[sbx] tailing /tmp/*.log (showing last 200 lines, then follow)')
  ...
  rl.on('line', async (line) => {
    ...
    if (trimmed === ':help') {
      ...
      console.log('  any other text  executes as `sh -lc "..."` inside the sandbox')
      rl.prompt()
      return
    }
    if (trimmed === ':tail') {
    ...
    const prefix = 'export BUN_INSTALL="$HOME/.bun"; export PATH="$BUN_INSTALL/bin:$PATH";'
