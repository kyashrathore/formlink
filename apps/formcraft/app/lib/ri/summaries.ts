...
export type SummaryRequest = {
  ...
  plan?: {
    rpc?: {
      submission_filters?: Record<string, unknown>
      answer_filters?: Record<string, unknown>
    }
...
type SummaryCacheMeta = {
  expiresAt?: string
  latest?: string | null
  version?: string
}

type RiAiCacheRow = Pick<Tables<"ri_ai_cache">, "meta" | "updated_at" | "value">
...
async function getLatestSubmissionAt(
...
function buildCacheKey(input: SummaryRequest, latest: string | null): string {
  ...
  const summariesOnly = (input.plan?.ui?.insights_spec || []).filter(
    (x) => x?.type === "summary"
  )
...
export async function getOrComputeSummary(
  ...
  try {
    const summariesOnly = (req.plan?.ui?.insights_spec || []).filter(
      (x) => x?.type === "summary"
    )
    const filtersForHash = {
      planRpc: req.plan?.rpc || {},
      search: req.search || {},
    }
    const specHash = sha256(stableStringifyDeep(summariesOnly))
  ...
  // Try cache
  const { data: cachedData, error: readErr } = await supabase
    .from("ri_ai_cache")
  ...
  const cached = (cachedData ?? null) as RiAiCacheRow | null
  if (cached) {
    const meta = (cached.meta as SummaryCacheMeta | null) ?? null
    const cachedValue = (cached.value as SummaryCacheValue | null) ?? null
    const expiresAtRaw = meta?.expiresAt
    const expiresAt =
      typeof expiresAtRaw === "string" ? expiresAtRaw : undefined
    const isExpired = expiresAt ? Date.now() > Date.parse(expiresAt) : false

    let summariesFromCache: Array<{ title?: string; content: string }> = []
    const candidateSummaries = cachedValue?.summaries
    const hasCachedSummaries = Array.isArray(candidateSummaries)
    if (hasCachedSummaries) {
      summariesFromCache = candidateSummaries as Array<{
        title?: string
        content: string
      }>
    }
  ...
  if (upsertErr) {
    logger.warn("[SUMMARY] Cache write error", {
      id,
  ...
  return {
    summaries,
    cache: { hit: false, key: id, updatedAt: new Date().toISOString() },
  }
}
...
async function refreshSummary(
  ...
  try {
    const summaries = await computeSummariesWithAI(supabase, req)
    const { error } = await supabase.from("ri_ai_cache").upsert({
      id,
      value: { summaries },
      meta: {
        latest,
    ...
    if (error)
      logger.warn("[SUMMARY] Refresh write error", { id, error: error.message })
...
const SummariesObjectSchema = z
  .object({
    summaries: z
      .array(
        z.object({ title: z.string().optional(), content: z.string().min(1) })
      )
      .min(1)
      .max(5),
  })
...
async function computeSummariesWithAI(
  supabase: SupabaseClient,
  req: SummaryRequest
) {
  // Load questions meta
  ...
  if (req.formVersionId) {
    try {
      ...
      questions = arr
        ...
        .map((x) => ({
          id: x.id as string,
          label: (x as any).title || (x as any).label,
        }))
    } catch {}
  }
  ...
  if (Array.isArray(submission_filters.status)) {
    ...
    if (submission_filters.status === undefined)
      delete (submission_filters as any).status
  }
  ...
  if (Array.isArray(submission_filters.testmode)) {
    ...
    if (arr.length === 1)
      submission_filters.testmode = String(arr[0]).toLowerCase() === "true"
    else delete (submission_filters as any).testmode
  } else if (typeof submission_filters.testmode === "string") {
    submission_filters.testmode =
      (submission_filters.testmode as string).toLowerCase() === "true"
  }
  ...
  const compactRows = rows.map((r) => {
    ...
    return {
      submission_id: (r as any).submission_id,
  ...
  const summariesSpecs = (req.plan?.ui?.insights_spec || []).filter(
    (x) => x.type === "summary"
  ) as SummarySpec[]
  ...
  try {
    ...
    return [
      {
        title: summariesSpecs[0]?.args?.title || "Summary",
        content: String(text),
      },
    ]
  }
