...
import { getGitAuthHeader } from "./config"
...
export async function pushChangesToBranch(
  ...
  try {
    ...
    const statusResult = await runCommandInSandbox(sandbox, "git", [
      "status",
      "--porcelain",
    ])
    ...
    // Commit changes
    const commitResult = await runCommandInSandbox(sandbox, "git", [
      "commit",
      "-m",
      commitMessage,
    ])
    ...
    await logger.updateStatus("pushing", `Pushing ${branchName} to origin`)
    const authHeader = getGitAuthHeader(githubToken)
    const pushArgs = authHeader
      ? ["-c", `http.extraHeader=${authHeader}`, "push", "origin", branchName]
      : ["push", "origin", branchName]

    await logger.command("git", pushArgs)

    const pushResult = await runCommandInSandbox(sandbox, "git", pushArgs)
    ...
    if (pushResult.success) {
      ...
      if (
        ...
        await logger.info(
          "You may need to check repository permissions or authentication tokens."
        )
      }
...
export async function shutdownSandbox(
  ...
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to shutdown sandbox"
