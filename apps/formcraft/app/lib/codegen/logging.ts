export type LogType = "info" | "error" | "command" | "success"
...
export function redactSensitiveInfo(message: string): string {
  ...
  const apiKeyPatterns = [
    ...
    // AI Gateway keys
    /AI_GATEWAY_API_KEY[=\s]*["']?([a-zA-Z0-9_-]{20,})/gi,
    // OpenAI API keys (sk-...)
    /OPENAI_API_KEY[=\s]*["']?([sk-][a-zA-Z0-9_-]{20,})/gi,
    // Codex API keys (generic sk- pattern)
    /CODEX_API_KEY[=\s]*["']?([a-zA-Z0-9_-]{20,})/gi,
    // GitHub tokens (ghp_, gho_, ghu_, ghs_, ghr_)
    /GITHUB_TOKEN[=\s]*["']?([gh][phosr]_[a-zA-Z0-9_]{20,})/gi,
    /CODEGEN_GITHUB_TOKEN[=\s]*["']?([gh][phosr]_[a-zA-Z0-9_]{20,})/gi,
    ...
    /CF_API_TOKEN[=\s:]*["']?([a-zA-Z0-9_-]{20,})/gi,
    /CF_ACCOUNT_ID[=\s:]*["']?([a-zA-Z0-9_-]{8,})/gi,
  ]
  ...
  redacted = redacted.replace(
    /"(teamId|projectId|accountId)"[\s:]*"([^"]+)"/gi,
    (match, fieldName) => {
