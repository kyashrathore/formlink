...
export function formatEventData(eventName: string, data: EventData): string {
  ...
  switch (eventName) {
    ...
    case "realtime_subscribed":
      return hasProperty(data, 'message') ? (data as any).message || JSON.stringify(data) : JSON.stringify(data)
    case "agent_init":
    ...
    case "agent_state_update": {
      ...
      else if (hasProperty(data, '_last_completed_node') && (data as any)._last_completed_node)
        summary.push(`Last Node: ${(data as any)._last_completed_node}`)
      if (hasProperty(data, 'formMetadata') && (data as any).formMetadata?.title)
      ...
      if (hasProperty(data, 'generatedQuestionSchemas') && (data as any).generatedQuestionSchemas)
        summary.push(`Questions: ${(data as any).generatedQuestionSchemas.length}`)
      if (hasProperty(data, 'journeyScript') && (data as any).journeyScript)
        summary.push("Journey script with strategy generated")
      if (hasProperty(data, 'resultPageGenerationPrompt') && (data as any).resultPageGenerationPrompt)
    ...
    case "task_completed": {
      ...
      if (eventName === "task_completed" && hasProperty(data, 'output') && (data as any).output) {
      ...
      if (eventName === "task_failed" && hasProperty(data, 'error') && (data as any).error) {
    ...
    case "agent_warning":
      if (hasProperty(data, 'details') && (data as any).details?.event_source === "metadata_generator_journey_script") {
        return (hasProperty(data, 'message') ? (data as any).message : '') || "Form journey script generated"
      }
      return (
        (hasProperty(data, 'message') ? (data as any).message : '') || `Warning: ${JSON.stringify(hasProperty(data, 'details') ? (data as any).details : data)}`
      )
