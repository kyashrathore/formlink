...
3.  **The `QuestionInputSwitcher`:**
    ...
    - It will use a `switch` statement on `question.display.inputType`.
    - For each `inputType`, it will render the corresponding component from our `@formlink/ui` library, passing the `field` props (`onChange`, `value`, `onBlur`, etc.) to it.

### 3.3.1 The Component Mapping (Comprehensive Guide)

This is the most critical part of the implementation. The `QuestionInputSwitcher` component must correctly map the `inputType` from our schema to the right component in our `@formlink/ui` library.

Here is the comprehensive mapping you should build.

| `question.display.inputType`                        | `@formlink/ui` Component            | `react-hook-form` Integration Notes                                                                                                                                                                                          |
| :-------------------------------------------------- | :---------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `text`, `email`, `url`, `tel`, `number`, `password` | `<Input />`                         | The `field` object can be spread directly: `<Input type={inputType} {...field} />`.                                                                                                                                          |
| `textarea`                                          | `<Textarea />`                      | The `field` object can be spread directly: `<Textarea {...field} />`.                                                                                                                                                        |
| `radio`                                             | `<RadioGroup />`                    | Use `onValueChange={field.onChange}` and `defaultValue={field.value}`. You will need to map over `question.options` to render the individual `<RadioGroupItem />` components inside.                                         |
| `checkbox`                                          | `<Checkbox />`                      | This is for a single boolean checkbox. Use `checked={field.value}` and `onCheckedChange={field.onChange}`.                                                                                                                   |
| `multiSelectDropdown`                               | `<Select />` (with `multiple` prop) | This is more complex. You'll use the `Select` components from `shadcn/ui` but need to manage the array state. The `field.value` will be an array of strings.                                                                 |
| `dropdown`                                          | `<Select />`                        | Use `onValueChange={field.onChange}` and `defaultValue={field.value}`. Map `question.options` to render `<SelectItem />` components.                                                                                         |
| `date`                                              | `<DatePicker />`                    | Our UI library should have a custom `DatePicker` that wraps the `shadcn/ui` Calendar. Use `selected={field.value}` and `onSelect={field.onChange}`.                                                                          |
| `dateRange`                                         | `<DateRangePicker />`               | Similar to `DatePicker`, but it will manage a `DateRange` object. The `field.value` will be an object with `from` and `to` properties.                                                                                       |
...
| `star` (from `rating`)                              | `<StarRating />`                    | A custom component. Use `onValueChange={field.onChange}` and `value={field.value}`.                                                                                                                                          |
| `linearScale`                                       | `<Slider />`                        | The `field` object can likely be passed directly. You may need to handle the value as an array (`[number]`) depending on the component's API.                                                                                |
