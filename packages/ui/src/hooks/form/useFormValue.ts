...
export function useFormValue<T = any>({
  ...
  const isPristine =
    JSON.stringify(externalValue) === JSON.stringify(initialValueRef.current);
    ...
    (value: T): string[] => {
      ...
      if (memoizedValidation) {
        ...
        if (typeof value === "string") {
          ...
          if (
            memoizedValidation.minLength &&
            value.length < memoizedValidation.minLength
          ) {
            errors.push(
              `Minimum length is ${memoizedValidation.minLength} characters`,
            );
          }

          // MaxLength validation
          if (
            memoizedValidation.maxLength &&
            value.length > memoizedValidation.maxLength
          ) {
            errors.push(
              `Maximum length is ${memoizedValidation.maxLength} characters`,
            );
          }

          // Pattern validation
          if (
            memoizedValidation.pattern &&
            !memoizedValidation.pattern.test(value)
          ) {
