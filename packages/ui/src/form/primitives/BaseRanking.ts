...
export function BaseRanking(props: BaseRankingProps): BaseRankingReturn {
  ...
  const reorder = useCallback(
    (fromIndex: number, toIndex: number) => {
      ...
      if (item) {
        setAnnouncement(
          `${item.label} moved from position ${fromIndex + 1} to position ${toIndex + 1}`,
        );
      }
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, value: string) => {
      if (disabled) return;
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetValue: string) => {
      if (disabled || !draggedItem) return;
