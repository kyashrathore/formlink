...
export function BaseDatePicker(props: BaseDatePickerProps): BaseDatePickerReturn {
  const {
    value,
    onChange,
    disabled = false,
  ...
  const isCalendarOpen = controlledIsCalendarOpen !== undefined ? controlledIsCalendarOpen : internalIsCalendarOpen;
  const setIsCalendarOpen = useCallback((open: boolean) => {
  ...
  }, [value, required, minDate, maxDate, disabledDates, formatDate, onValidate]);
  ...
  }, [disabled, enableKeyboard, isCalendarOpen, highlightedDate, selectDate, openCalendar, closeCalendar]);

  const handleCalendarKeyDown = useCallback((event: React.KeyboardEvent) => {
  ...
  }, [disabled, enableKeyboard, isCalendarOpen, highlightedDate, currentMonth, selectDate, closeCalendar]);
  ...
  const getDaysInMonth = useCallback((): CalendarDay[] => {
    ...
    for (let day = 1; day <= lastDay.getDate(); day++) {
      ...
      const isHighlighted = date.toDateString() === highlightedDate.toDateString();
    ...
    return days;
  }, [currentMonth, value, highlightedDate, firstDayOfWeek, isDateDisabled, selectDate]);
