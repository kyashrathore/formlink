...
> = (props) => {
  ...
  const inputClasses = [theme?.textInput?.input].filter(Boolean).join(" ");
...
describe("BaseTextInput", () => {
    ...
    it("renders different input types", () => {
      ...
      types.forEach((type) => {
        const { rerender } = render(
          <TestComponent {...defaultProps} type={type} />,
        );
        const input = screen.getByPlaceholderText("Enter text");
  ...
  describe("Validation", () => {
    ...
    it("displays error message", () => {
      render(
        <TestComponent {...defaultProps} error errorMessage="Invalid input" />,
      );
    ...
    it("calls onValidate when provided", async () => {
      ...
      const { rerender } = render(
        <TestComponent {...defaultProps} onValidate={onValidate} value="" />,
      );
  ...
  describe("Advanced Features", () => {
    ...
    it("debounces onChange when specified", async () => {
      ...
      render(
        <TestComponent {...defaultProps} onChange={onChange} debounce={300} />,
      );
