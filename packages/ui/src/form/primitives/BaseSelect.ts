...
export function BaseSelect<T = string>(props: BaseSelectProps<T>): BaseSelectReturn<T> {
  const {
    value,
    onChange,
    options,
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    ...
    if (isOpen) {
      ...
      if (event.key === 'Enter' || event.key === ' ') {
        ...
        event.preventDefault();
        open();
      } else if (enableShortcuts && !event.shiftKey) { // Don't handle shortcuts if shift is pressed
        // Safe array access with fallback
        const safeOptions = options || [];
  ...
  }, [disabled, isOpen, highlightedIndex, options, enableShortcuts, enableArrowNavigation, open, close, selectByIndex]);
