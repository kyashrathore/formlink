...
import { BaseLinearScale, LinearScaleConfig } from '../../primitives/BaseLinearScale';
...
export function UnifiedLinearScale({
  ...
  const shouldShowKeyboardHints = showKeyboardHints ?? (mode === 'typeform' ? true : false);
  ...
  const innerContainerClass = mode === 'chat' ? "flex flex-col gap-4" : "flex flex-col gap-6";
  const buttonsContainerClass = mode === 'chat' 
    ? "flex gap-2 justify-center" 
    : "flex gap-3 justify-start";
  const buttonClass = mode === 'chat'
    ? cn(
  ...
  return (
    <div className={containerClass}>
      <div 
        {...containerProps} 
        onKeyDown={mode === 'chat' ? handleContainerKeyDown : containerProps.onKeyDown}
      ...
      >
        ...
        <div className={buttonsContainerClass}>
          {scaleValues.map((scaleValue) => (
            <button
              ...
              className={cn(
                buttonClass,
                isSelected(scaleValue) && "border-primary bg-primary/10 hover:bg-primary/15",
                disabled && "opacity-50 cursor-not-allowed hover:bg-card/50 hover:border-border/50"
              )}
            >
              {scaleValue}
            </button>
        ...
        {(config.startLabel || config.endLabel) && (
          <div className={labelsClass}>
            ...
            {config.endLabel && <span className="text-right">{config.endLabel}</span>}
          </div>
        )}
      </div>
