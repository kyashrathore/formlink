...
export class AIThemeExtractorImpl implements AIThemeExtractor {
  ...
  enhanceTheme(partialTheme: PartialTheme): FormJunctionTheme {
    ...
    const enhancedTheme = JSON.parse(
      JSON.stringify(defaultTheme),
    ) as FormJunctionTheme;
    ...
    if (partialTheme.tokens?.colors?.primary) {
      ...
      if (!enhancedTheme.tokens.colors.primaryHover) {
        enhancedTheme.tokens.colors.primaryHover = this.adjustBrightness(
          primary,
          -20,
        );
      }
      if (!enhancedTheme.tokens.colors.primaryActive) {
        enhancedTheme.tokens.colors.primaryActive = this.adjustBrightness(
          primary,
          -40,
        );
      }
  ...
  private extractColors(crawlData: CrawlResult): any {
    ...
    if (crawlData.styles.buttons.length > 0) {
      const primaryButton =
        crawlData.styles.buttons.find((b) => b.isPrimary) ||
        crawlData.styles.buttons[0];
    ...
    colors.backgroundAlt =
      crawlData.styles.background.secondary ||
      this.adjustBrightness(colors.background, 5);

    // Extract text colors
    colors.text = crawlData.styles.text.primary;
    colors.textMuted =
      crawlData.styles.text.secondary || this.adjustBrightness(colors.text, 40);
  ...
  private extractEffects(crawlData: CrawlResult): any {
    return {
      shadowMD:
        crawlData.styles.shadows.medium || "0 4px 6px rgba(0, 0, 0, 0.1)",
