...
export class ThemeEngine {
  ...
  applyPartialTheme(partialTheme: PartialTheme): ValidationResult {
    ...
    const mergedTheme = this.deepMerge(this.currentTheme, partialTheme) as FormlinkTheme;
  ...
  private applyAccessibilitySettings(theme: FormlinkTheme): void {
    ...
    if (accessibility.fontSizeMultiplier !== 1) {
      root.style.setProperty('--fj-font-size-multiplier', String(accessibility.fontSizeMultiplier));
    }
