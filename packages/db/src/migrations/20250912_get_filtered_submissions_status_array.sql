-- Update get_filtered_submissions to support status array in submission_filters
...
BEGIN
  ...
  WITH filtered_submissions_base AS (
    ...
    WHERE
      ...
      AND (
        ...
        (submission_filters ? 'status') IS FALSE
        OR (
          (jsonb_typeof(submission_filters->'status') = 'array' AND fs_inner.status IN (
            SELECT value::public.submission_status FROM jsonb_array_elements_text(submission_filters->'status')
          ))
          OR (
            (jsonb_typeof(submission_filters->'status') <> 'array' AND submission_filters->>'status' IS NOT NULL AND fs_inner.status = (submission_filters->>'status')::public.submission_status)
          )
        )
      )
  ...
  answer_conditions AS (
    SELECT DISTINCT a.submission_id, a.question_id
    FROM form_answers a
  ...
  submissions_meeting_filter_criteria AS (
    ...
    GROUP BY fsb.original_submission_id, fsb.form_version_id, fsb.user_id, fsb.created_at, fsb.completed_at, fsb.status, fsb.testmode
  ),
  answer_filter_stats AS (
    SELECT CASE WHEN answer_filters IS NOT NULL AND answer_filters != '{}'::jsonb
      THEN (SELECT count(*) FROM jsonb_object_keys(answer_filters))::integer ELSE 0 END AS total_filter_keys
  ),
  qualified_submissions AS (
    SELECT smfc.original_submission_id, smfc.form_version_id, smfc.user_id, smfc.created_at, smfc.completed_at, smfc.status, smfc.testmode
    FROM submissions_meeting_filter_criteria smfc
    CROSS JOIN answer_filter_stats afs
    WHERE (afs.total_filter_keys = 0 OR smfc.matched_question_count = afs.total_filter_keys)
  ),
  aggregated_answers AS (
    SELECT fa.submission_id, jsonb_object_agg(fa.question_id, fa.answer_value) as submission_answers
  ...
  SELECT COALESCE((SELECT jsonb_agg(row_to_json(pr.*)) FROM paginated_results pr), '[]'::jsonb) AS data,
    tc.total AS total_count, tc.total_completed AS total_completed_count, tc.total_in_progress AS total_in_progress_count,
    c.total_filtered AS total_filtered_count, c.completed AS completed_count, c.in_progress AS in_progress_count
  FROM counts c CROSS JOIN total_counts tc;
END;
$$ LANGUAGE plpgsql;
